...
- `src/main.js` — bootstrap: Opening (menu.js) → Character Creation → build world → game loop; wires the interact system (F key / mobile ★: talk / chest / harvest / plant / build / chop / mine / cook / fish), auto-aim, jump (Space), mounts (M), world map (N), AFK fishing (G), world boss scheduler (180s), safe zones (village/camps/homes — `inSafeZone`), economy/cooking/estate/gacha APIs, out-of-combat auto-heal, save v3
...
- `src/world/camps.js` — 4 rest camps (campfire+tent+bench): safe zones, fast heal, cooking stations
...
- `src/systems/skills.js` — 12 skills with cooldowns & juicy FX (shockwaves, light flashes, screen shake) + skill leveling (Lv1-5: +22% power, -6% cooldown, longer buffs per level; 1 skill point per character level, spent in the K panel)
...
- `src/systems/mounts.js` — 6 rideable mounts: Sprig (starter, free), Trotter/Clucky/Shellsworth/Nimbus (quests), Blossom (gacha-exclusive, petal trail); mesh attaches under the player, gallop anim, speed/jump multipliers
...
- `src/entities/npcs.js` — Riverbrook village: 5 huts (door steps, lanterns, flower boxes), well, market stall, forge corner, fenced garden, flower beds, lamps, community campfire (cooking), gacha capsule machine; 10 villagers: 5 quest-givers + Master NXR (koala Gacha Master modeled after the creator's mascot: fluffy ears, NXR VR visor, golden batik headband/sash) + 4 ambient (squirrel kid, turtle grandpa, duck, dog) with activity schedules. IMPORTANT: village props must never block the 3x3 cells around terrain.spawn (a well footprint once trapped the player at spawn — cells are force-cleared at the end of the village build)
...
- `src/world/terrain.js`, `decor.js`, `water.js` — SMOOTH heightfield terrain (bilinear corner grid, faceted low-poly shading, calm Stardew-ish tiles, soft path-edge fringe tiles — deliberately NOT blocky), flattened village pocket; scenery trees are PIXEL-ART IMPOSTERS (crossed sprite planes + round scalloped top cap, short & sparse so the steep camera never loses the player), faceted rocks, dense flower meadows, bushes, torches, butterflies (day) & fireflies (night); lakes are NOT walkable

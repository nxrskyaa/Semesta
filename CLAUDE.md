...
- `src/entities/npcs.js` — Riverbrook village: 5 huts (door steps, lanterns, flower boxes), well, market stall, forge corner (anvil+furnace glow), fenced garden, flower beds, lamps, clutter; animal villagers walk activity schedules (hammering w/ sparks, fishing, garden tending, shopkeeping, patrols), greet the player, quest markers. IMPORTANT: village props must never block the 3x3 cells around terrain.spawn (a well footprint once trapped the player at spawn — cells are force-cleared at the end of the village build)
